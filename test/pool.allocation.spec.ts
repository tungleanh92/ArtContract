...
describe("Pool", () => {
  ...
  context("With ERC/LP token added to the field", async () => {
    ...
    it("should allow emergency withdraw", async () => {
      ...
      await distributeToken.transfer(pool.address, "10000000000000000");
    ...
    it("should give out TOKENs only after farming time", async () => {
      ...
      expect(await distributeToken.balanceOf(account1.address)).to.equal("1000");

      await time.advanceBlockTo(104);
      await pool.connect(account1).deposit(["0"]); // block 105


      expect(await distributeToken.balanceOf(account1.address)).to.equal("5000");
    ...
    it("should distribute TOKENs properly for each staker", async () => {
      ...
      //   user 1 should have: 4*1000 + 4*1/3*1000 + 2*1/6*1000 = 5666

      await time.advanceBlockTo(219);
      await pool.connect(account1).deposit(["10"]); // block 220

      expect(await distributeToken.balanceOf(account1.address)).to.equal("5666");
      ...
      //   user 2 should have: 4*2/3*1000 + 2*2/6*1000 + 10*2/7*1000 = 6190
      await time.advanceBlockTo(229);
      await pool.connect(account2).withdraw(["5"]);
      expect(await distributeToken.balanceOf(account1.address)).to.equal("5666");
      expect(await distributeToken.balanceOf(account2.address)).to.equal("6190");
      expect(await distributeToken.balanceOf(account3.address)).to.equal("0");
      ...
      // user 1 should have: 5666 + 10*2/7*1000 + 10*2/6.5*1000 = 11600
      expect(await distributeToken.balanceOf(account1.address)).to.equal("11600");
      // user 2 should have: 6190 + 10*1.5/6.5 * 1000 + 10*1.5/4.5*1000 = 11831
      expect(await distributeToken.balanceOf(account2.address)).to.equal("11831");
      // user 3 should have: 2*3/6*1000 + 10*3/7*1000 + 10*3/6.5*1000 + 10*3/4.5*1000 + 10*1000 = 26568
      expect(await distributeToken.balanceOf(account3.address)).to.equal("26568");
    ...
    it("should give proper TOKENs allocation to each pool", async () => {
      ...
      // user 1 should have 10*1000/3 pending reward
      expect((await pool.pendingToken(account1.address))[0].toString()).to.equal("3330");
      await distributeToken2.transfer(pool2.address, "10000000000000000");
      ...
      // user 1 should have 10*1000/3  + 5*1/3*1000 = 5000 pending reward
      expect((await pool.pendingToken(account1.address))[0].toString()).to.equal("5000");

      // At block 330. user 2 should get 5*2/3*1000 ~ 3330. user 1 should get ~1666 more.
      await time.advanceBlockTo(330);

      expect((await pool2.pendingToken(account2.address))[0].toString()).to.equal("3330");
      expect((await pool.pendingToken(account1.address))[0].toString()).to.equal("6660");
    ...
    it("should give proper TOKENs allocation to multi rewards pool", async () => {
      ...
      await distributeToken.transfer(pool2.address, "10000000000000000");
      await distributeToken2.transfer(pool2.address, "10000000000000000");
      ...
      await pool2.connect(account1).deposit(["10", "10"]);

      await time.advanceBlockTo(419);
      await pool2.connect(account1).deposit(["0", "0"]);

      expect(await distributeToken.balanceOf(account1.address)).to.equal("1666")
      expect(await distributeToken2.balanceOf(account1.address)).to.equal("1666")
